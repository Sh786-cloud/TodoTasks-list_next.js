...
export default function Todolist() {
    const [tasks, setTasks] = useState<Task[]>([]); // eslint-disable-next-line @typescript-eslint/no-empty-object-type
    const [newTask, setNewTask] = useState<string>("");
    const [editingTaskId, setEditingTaskId] = useState<number | null>(null);
    const [editedTaskText, setEditedTaskText] = useState<string>("");
    const [isMounted, setIsounted] = useState<boolean>(false);
    ...
    const updateTask = (): void => {
        if (editedTaskText.trim() !== "") {
            setTasks(
                tasks.map((task) => task.id === editingTaskId ? { ...task, text: editedTaskText } : task)
            )
        }
    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen bg-gray-100 dark:bg-gray-900">
            <div className="w-full max-w-md bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6">
                ...
                <div className="space-y-2">
                    {tasks.map((task) => (
                        <div
                            key={task.id}
                            className="flex justify-between bg-gray-200 dark:bg-gray-700 rounded-md px-4 py-2"
                        >
                            ...
                            <div className="flex items-center">
                                ...
                                <Button
                                    ...
                                    className="bg-red-500 hover:bg-red-700 text-white font-medium py-1 px-2 rounded-md"
                                >
                                    Delete
                                </Button>
                            </div>
            ...
            <div className="text-blue-500 font-medium flex items-center mt-3 border border-orange-400 rounded-sm px-3">
                Designed by : Engr M Shahzeb
            </div>
        </div>
    )
